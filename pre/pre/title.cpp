...
#define	TITLE_LOGO_POS_Y		(260)		// タイトルロゴの位置(Y座標)
#define	TITLE_LOGO_WIDTH		(600)		// タイトルロゴの幅
#define	TITLE_LOGO_HEIGHT		(450)		// タイトルロゴの高さ

#define	START_POS_X				(350)		// スタートボタンの位置(X座標)
#define	START_POS_Y				(720)		// スタートボタンの位置(Y座標)
#define	START_WIDTH				(480)		// スタートボタンの幅
#define	START_HEIGHT			(120)		// スタートボタンの高さ

#define ALPHA					(0.05)		// アルファ値の変化値
...
{
	LPDIRECT3DDEVICE9 pDevice = GetDevice();
	D3DXMATRIX mtxView, mtxScl, mtxRot, mtxTranslate;
	...
	// 回転を反映
	D3DXMatrixRotationYawPitchRoll(&mtxRot, 0.0, 0.0f, 0.0f);
